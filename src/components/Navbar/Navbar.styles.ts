...
export const Button = styled.button`
  border-radius: ${({ bigRadius }: IBtnProps) => (bigRadius ? "30px" : "20px")};
  background-color: ${({ primary }: IBtnProps) =>
    primary ? "#E38B06" : "#000"};
  color: ${({ primary }: IBtnProps) => (primary ? "#000" : "#fff")};
  padding: ${({ big }: IBtnProps) => (big ? "18px 30px" : "10px 28px")};
  font-size: ${({ bigFont }: IBtnProps) => (bigFont ? "20px" : "18px")};
  outline: none;
  cursor: pointer;
  border: none;
